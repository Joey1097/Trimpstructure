...
- 世界观背景
  - 在无尽的"结构宇宙"中，玩家扮演一位"架构师"，通过采集资源、探索节点、构建神器，逐步重建遗落文明的"核心结构"。
  - 每次 Prestige 代表一次"维度跃迁"，在更高层的结构中开启新的可能性。
  - 世界节点是古代文明留下的"知识碎片"，神器则是被封印的"力量结晶"。
- 设计目标
  - 低门槛上手：首日内可见显著成长拐点。
...
- 新手节奏表（前60分钟）
  | 时间 | 里程碑 | 解锁内容 |
  |------|--------|----------|
  | 0-2分钟 | 第一次资源采集 | 工人岗位、木/石资源 |
  | 5分钟 | 人口增长 | 建造岗位 |
  | 10分钟 | 首个建筑升级 | 仓库、基础产出加成 |
  | 15分钟 | 完成第一张地图 | 士兵岗位、蓝图货币 |
  | 30分钟 | 解锁世界树 | 世界节点系统、记忆货币 |
  | 45分钟 | 第一个世界节点 | 全局乘数加成 |
  | 60分钟 | 获得第一个神器 | 神器系统、装备槽 |
- 目标用户与平台
...
- 地图
  ...
  - 机制类型：
    ...
    - 传送门：允许跳跃式推进，跳过部分节点直达深处。
    - 事件节点：随机触发奖励或惩罚事件，增加探索趣味。
    - 精英节点：高难高收益可选挑战，提供稀有掉落。
...
- 多层 Prestige 系统
  - Layer 1 - 传承（约2-4小时触发）：
    - 条件：达成若干章节终点或完成挑战里程碑
    - 回报：传承点，购买效率乘数、解锁便利功能
    - 保留：世界节点进度、神器收藏
  - Layer 2 - 觉醒（约1-2天触发）：
    - 条件：累计传承次数或达成特定成就
    - 回报：觉醒晶体，解锁新流派、新神器标签、高阶岗位
    - 保留：觉醒永久加成、成就
  - Layer 3 - 超越（长期目标）：
    - 条件：完成所有觉醒成就或挑战全通关
    - 回报：循环碎片，永久突破数值软顶、解锁隐藏内容
    - 保留：全部永久进度
- 挑战关
...
F. 日常循环与成就
- 日常系统
  - 每日登录：领取离线收益 + 当日目标清单（3个随机任务）
  - 每日挑战：限定规则的速通地图，完成奖励挑战货币
  - 连续登录：7天周期奖励，包含神器碎片和稀有材料
- 每周循环
  - 周常Boss：高难限时节点，本地最佳成绩记录
  - 周常目标：完成特定数量的地图/战斗/研究，奖励传承点
- 成就系统
  - 探索类：发现X个节点、通关X张地图、解锁所有区域
  - 效率类：单次产出达到X、战斗力达到Y、离线收益达到Z
  - 收藏类：收集X个神器、解锁所有世界节点、尝试所有流派
  - 挑战类：限时通关、无伤通关、特殊条件通关
  - 奖励：成就点数、外观解锁、称号、永久微加成（0.5%-2%）

四、数值与平衡指南
- 增长与边际
  - 单系统内递减：对数/根式；跨系统采用加法基础+乘法修正混合模型
  - 公式示例：最终产出 = (基础值 + 加法加成) × (1 + 乘法加成总和) × 外部乘数
  - 控墙策略：机制墙（护盾/路径/限时）优先，数值墙谨慎使用
- 数值平衡参考表
  | 游戏时间 | 资源/秒 | 战斗力 | 期望层级 |
  |----------|---------|--------|----------|
  | 1小时 | 10-50 | 100-300 | 首次传承前 |
  | 4小时 | 500-2K | 2K-5K | 1-2次传承 |
  | 1天 | 10K-50K | 20K-100K | 3-5次传承 |
  | 1周 | 1M-10M | 1M-5M | 首次觉醒 |
  | 1月 | 1B+ | 100M+ | 多次觉醒 |
- TTK与节奏
...
六、前端架构设计
A. 架构取向
- 纯本地单机：全部核心逻辑在前端执行，无需联网。
- 离线友好：支持完全离线游玩，数据存储于本地浏览器。
...
九、内容配置
- 数据驱动
  - 地图、世界树、神器、研究、建筑均用独立 JSON 表配置。
- 本地化
  - 采用键值对资源文件；UI 文本与数值单位可替换。

十、可测试性与质量保障
- 单元测试
  - formulas、sim、progression、save/restore 全覆盖。
- 断言与卫语句
  - 核心 tick 路径对输入/输出范围进行断言，异常立刻降级处理。
- 可视化测试
